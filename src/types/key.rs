...
use super::{
    scale::ScaleDegree, Accidental, Chord, HarmonicFunction, Letter, NoteName, RomanNumeral, Scale,
};
...
impl Key {
    ...
    pub fn accidental_to_display(&self, note: NoteName) -> Option<Accidental> {
        ...
        let actual = if note.accidental() == Accidental::Natural {
            None
        } else {
    ...
    fn find_node_for_chord(&self, chord: &Chord) -> Option<crate::types::RomanChord> {
        ...
        for node in &nodes {
            if node.root == roman_chord.root() && node.bass == roman_chord.bass {
                ...
                let chord_intervals: std::collections::HashSet<_> =
                    roman_chord.intervals().iter().copied().collect();
