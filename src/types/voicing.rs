...
use crate::{types::{Chord, Pitch, Interval, NoteName}, traits::{HasIntervals, HasRoot}, error::ParseError};
...
impl PitchRange {
    ...
    pub fn contains(&self, pitch: Pitch) -> bool {
        pitch.midi_number() >= self.low.midi_number() && pitch.midi_number() <= self.high.midi_number()
    }
...
impl VoicingStyle {
    ...
    pub fn spread(min_interval: Interval, max_interval: Interval) -> Self {
        Self::Spread { min_interval, max_interval }
    }
}
...
impl Voicer {
    ...
    pub fn voice_chord_from_pitch(&self, chord: &Chord, bass_pitch: Option<Pitch>) -> Result<VoicedChord, VoicingError> {
        ...
        match self.config.style {
            ...
            VoicingStyle::Spread { min_interval, max_interval } => {
    ...
    fn get_starting_pitch(&self, chord: &Chord, bass_pitch: Option<Pitch>) -> Pitch {
        ...
        loop {
            ...
            if attempts > MAX_ATTEMPTS {
                ...
                let range_middle_midi = (self.config.range.low.midi_number() as i16 + self.config.range.high.midi_number() as i16) / 2;
            ...
            if candidate_root.midi_number() > self.config.range.high.midi_number() - 12 && octave > -2 {
                octave -= 1;
                continue;
            }
    ...
    fn voice_closed(&self, chord: &Chord, bass_pitch: Option<Pitch>) -> Result<VoicedChord, VoicingError> {
    ...
    fn voice_open(&self, chord: &Chord, bass_pitch: Option<Pitch>) -> Result<VoicedChord, VoicingError> {
        let root_pitch = self.get_starting_pitch(chord, bass_pitch);
        let intervals = chord.intervals();
    ...
    fn voice_drop2(&self, chord: &Chord, bass_pitch: Option<Pitch>) -> Result<VoicedChord, VoicingError> {
        ...
        if pitches.iter().any(|p| p.midi_number() < self.config.range.low.midi_number()) {
            return Err(VoicingError::OutOfRange);
        }
    ...
    fn voice_drop3(&self, chord: &Chord, bass_pitch: Option<Pitch>) -> Result<VoicedChord, VoicingError> {
        // Start with closed voicing
        let closed = self.voice_closed(chord, bass_pitch)?;
        let mut pitches = closed.pitches.clone();
    ...
    fn voice_spread(&self, chord: &Chord, bass_pitch: Option<Pitch>, min_interval: Interval, max_interval: Interval) -> Result<VoicedChord, VoicingError> {
        let root_pitch = self.get_starting_pitch(chord, bass_pitch);
        let intervals = chord.intervals();
        ...
        let info = VoicingInfo::new(VoicingStyle::Spread { min_interval, max_interval }, self.config.range, 0);
        Ok(VoicedChord::new(chord.clone(), pitches, info))
    }
...
impl FromStr for StringState {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            fret_str => {
                let fret = fret_str.parse::<u8>()
                    .map_err(|_| ParseError::InvalidChordFormat(format!("Invalid fret number: {}", fret_str)))?;
                if fret > 24 {
                    return Err(ParseError::InvalidChordFormat(format!("Fret number too high: {}", fret)));
                }
...
impl GuitarFingering {
    ...
    pub fn new(frets: [StringState; 6], root_string: u8) -> Result<Self, ParseError> {
        if root_string > 5 {
            return Err(ParseError::InvalidChordFormat(format!(
                "Root string index {} is invalid, must be 0-5", root_string
            )));
        }
        ...
        for fret_state in &mut new_frets {
            if let StringState::Fretted(fret) = fret_state {
                ...
                if new_fret < 0 || new_fret > 24 {
                    return Err(ParseError::InvalidChordFormat(format!(
                        "Transposition would result in invalid fret: {}", new_fret
                    )));
                }
    ...
    pub fn with_muted_to_open_variations(&self) -> Vec<Self> {
        ...
        let muted_count = self.frets.iter().filter(|s| matches!(s, StringState::Muted)).count();
        if muted_count > 1 {
...
impl FromStr for GuitarFingering {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if parts.len() != 2 {
            return Err(ParseError::InvalidChordFormat(format!(
                "Guitar fingering format should be 'fingering,root_string', got: {}", s
            )));
        }
        
        let fingering_str = parts[0];
        let root_string: u8 = parts[1].parse()
            .map_err(|_| ParseError::InvalidChordFormat(format!(
                "Invalid root string index: {}", parts[1]
            )))?;
        
        if fingering_str.len() != 6 {
            return Err(ParseError::InvalidChordFormat(format!(
                "Guitar fingering must have exactly 6 characters, got: {}", fingering_str
            )));
        }
...
impl IntervalFirstGuitarFinder {
    ...
    fn try_all_combinations(
        ...
        if string_index >= strings.len() {
            if let Ok(fingering) = GuitarFingering::new(current_frets, bass_string) {
                ...
                let coverage = required_notes.iter()
    ...
    fn has_playable_fret_span(&self, fingering: &GuitarFingering) -> bool {
        ...
        let fretted_positions: Vec<u8> = frets.iter()
    ...
    fn matches_known_shape(&self, fingering: &GuitarFingering) -> bool {
        ...
        for (i, shape) in self.shapes.iter().enumerate() {
            if self.matches_shape_with_open_extensions(&extracted_shape, shape) {
                println!("      Matches shape {}: {:?} (with open extensions)", i, shape.positions);
                return true;
            }
        }
    ...
    fn matches_shape_with_open_extensions(&self, extracted_pattern: &[u8], shape: &GuitarShape) -> bool {
    ...
    pub fn find_voicings(&self, chord: &Chord) -> Vec<(GuitarFingering, f32)> {
        ...
        for bass_pos in bass_positions {
            println!("Trying bass position: string {}, fret {}", bass_pos.0, bass_pos.1);
            ...
            for (i, candidate) in candidates.iter().enumerate() {
                ...
                if candidate.root_pitch(&self.tuning).is_some() {
                    ...
                    println!("    Pitches: {:?}", pitches);
                    println!("    Notes: {:?}", pitches.iter().map(|p| p.name).collect::<Vec<_>>());
        ...
        if !all_voicings.is_empty() {
            ...
            println!("Shape match: {}", if shape_match { "YES" } else { "NO (algorithmic)" });
            println!("Pitches: {:?}", best_pitches);
            println!("Notes: {:?}", best_pitches.iter().map(|p| p.name).collect::<Vec<_>>());
    ...
    fn calculate_score_for_chord(&self, chord: &Chord, _bass_pos: (u8, u8), fingering: &GuitarFingering) -> f32 {
        let mut score = 0.0;
        let required_notes = self.get_chord_notes(chord);
...
mod test_compat {
    ...
    impl GuitarChordAnalyzer {
        ...
        pub fn find_matches(&self, chord: &Chord) -> Vec<GuitarChordMatch> {
            ...
            voicings.into_iter().map(|(fingering, score)| {
                let pitches = fingering.to_pitches(&self.tuning);
                GuitarChordMatch {
                    fingering,
...
mod tests {
    ...
    fn test_voicer_spread_voicing() {
        let chord = Chord::major(note!("C"));
        let config = VoicingConfig::new()
            .style(VoicingStyle::spread(Interval::MAJOR_THIRD, Interval::PERFECT_FIFTH))
            .range_from("C3".parse().unwrap(), "C6".parse().unwrap());
        let voicer = Voicer::new(config);
    ...
    fn test_string_state_parsing() {
        ...
        assert_eq!("12".parse::<StringState>().unwrap(), StringState::Fretted(12));
    ...
    fn test_guitar_chord_analyzer_basic() {
        ...
        assert!(!voicings.is_empty(), "Should find voicings for C major chord");
        
        // The best voicing should have a reasonable score (lower is better)
        let (best_fingering, best_score) = &voicings[0];
        assert!(*best_score < 50.0, "Best voicing score should be reasonable: {}", best_score);
    ...
    fn test_guitar_chord_analyzer_different_chords() {
        ...
        for chord in &test_chords {
            let voicings = finder.find_voicings(chord);
            assert!(!voicings.is_empty(), "Should find voicings for chord: {}", chord);
    ...
    fn test_guitar_voicing_style_integration() {
        ...
        assert!(guitar_voiced.is_ok(), "Guitar voicing should succeed for C major");
        ...
        assert!(!voiced_chord.pitches.is_empty(), "Should produce guitar voicing pitches");
