...
macro_rules! guitar_voicing_test {
    ($test_name:ident, $chord_expr:expr, $expected_pitches:expr) => {
        ...
        fn $test_name() {
            ...
            let expected: Vec<Pitch> = $expected_pitches.split(',')
                .map(|s| s.trim().parse().unwrap())
                .collect();
            
            // Main assertion
            assert_eq!(voiced.pitches, expected, 
                      "Guitar voicing for {} does not match expected pitches", chord);
            
            // Additional validations
            assert_eq!(voiced.info.style, VoicingStyle::Guitar, 
                      "Should use Guitar voicing style");
            assert!(!voiced.pitches.is_empty(), 
                    "Voicing should produce pitches");
            
            // Check guitar range (E2 to E6)
            for pitch in &voiced.pitches {
                assert!(pitch.midi_number() >= 40 && pitch.midi_number() <= 88, 
                       "Pitch {} outside guitar range (E2-E6)", pitch);
            }
        }
    };
}
...
guitar_voicing_test!(test_e_minor_guitar_voicing, 
    Chord::minor(note!("E")), 
    "E2,B2,E3,G3,B3,E4");

guitar_voicing_test!(test_e_major_guitar_voicing,
    Chord::major(note!("E")),
    "E2,B2,E3,G#3,B3,E4");

guitar_voicing_test!(test_e7_guitar_voicing,
    Chord::dominant_7th(note!("E")),
    "E2,B2,D3,G♯3,B3,E4");

guitar_voicing_test!(test_a_minor_guitar_voicing,
    Chord::minor(note!("A")),
    "A2,E3,A3,C4,E4");

guitar_voicing_test!(test_a_major_guitar_voicing,
    Chord::major(note!("A")),
    "A2,E3,A3,C#4,E4");

guitar_voicing_test!(test_c_major_guitar_voicing,
    Chord::major(note!("C")),
    "C3,E3,G3,C4,E4");

guitar_voicing_test!(test_g_major_guitar_voicing,
    Chord::major(note!("G")),
    "G2,B2,D3,G3,B3,G4");

guitar_voicing_test!(test_d_major_guitar_voicing,
    Chord::major(note!("D")),
    "D3,A3,D4,F♯4");

guitar_voicing_test!(test_b7_guitar_voicing,
    Chord::dominant_7th(note!("B")),
    "B2,D♯3,A3,B3");

guitar_voicing_test!(test_dmaj7_guitar_voicing,
    Chord::major_7th(note!("D")),
    "D3,A3,C#4,F#4");

guitar_voicing_test!(test_d7_guitar_voicing,
    Chord::dominant_7th(note!("D")),
    "D3,A3,C4,F#4");

// Additional common chord voicings  
guitar_voicing_test!(test_f_major_guitar_voicing,
    Chord::major(note!("F")),
    "F2,C3,F3,A3,C4,F4");

guitar_voicing_test!(test_g_minor_guitar_voicing,
    Chord::minor(note!("G")),
    "G2,D3,G3,A#3,D4,G4");

guitar_voicing_test!(test_b_minor_guitar_voicing,
    Chord::minor(note!("B")),
    "B2,F♯3,B3,D4,F♯4,B4");

// Slash chord tests - verifying bass note handling
guitar_voicing_test!(test_c_major_over_e,
    Chord::major(note!("C")).with_slash_bass(note!("E")),
    "E2,C3,E3,G3,C4,E4");

guitar_voicing_test!(test_c_minor_over_e,
    Chord::minor(note!("C")).with_slash_bass(note!("E")),
    "E2,C3,G3,C4,D♯4,G4");

guitar_voicing_test!(test_a_major_over_e,
    Chord::major(note!("A")).with_slash_bass(note!("E")),
    "E2,A2,E3,A3,C#4,E4");

guitar_voicing_test!(test_d_major_over_a,
    Chord::major(note!("D")).with_slash_bass(note!("A")),
    "A2,D3,A3,D4,F#4");

guitar_voicing_test!(test_c_major_over_g,
...
fn test_cm_over_e_slash_chord() {
    ...
    let has_eb = pitches.iter().any(|p| p.name == note!("E♭") || p.name == note!("D♯"));
    ...
    println!("Cm/E best voicing: Score {:.2}, Pattern: {:?}", best_score, best_fingering);
...
fn test_interval_first_approach() {
    ...
    for (i, (fingering, score)) in voicings.iter().take(3).enumerate() {
        ...
        println!("  {}. Score: {:.2}, Fingering: {:?}", i+1, score, fingering);
        println!("     Pitches: {:?}", pitches);
    }
