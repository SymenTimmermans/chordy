...
fn test_progressions_from_include_variants() {
    ...
    assert!(
        ...
        "I should have strong progression options"
    );
    ...
    let has_seventh_chords = options
        .strong
        .iter()
        .chain(options.moderate.iter())
        .chain(options.weak.iter())
        .any(|chord| {
            chord.intervals.contains(Interval::MINOR_SEVENTH)
                || chord.intervals.contains(Interval::MAJOR_SEVENTH)
        });
    assert!(
        has_seventh_chords,
        "Should include seventh chords somewhere in the progression options"
    );
}
...
fn test_chord_progressions_from_categorization() {
    ...
    assert!(
        ...
        "Should have moderate progressions"
    );
...
fn test_chord_progressions_from_different_keys() {
    ...
    assert!(
        has_c_progressions || has_d_progressions,
    ...
    if !has_d_progressions {
        let has_d_in_moderate = options
            .moderate
            .iter()
            .any(|chord| chord.root == note!("D"));
        assert!(
            has_d_in_moderate,
            "I in G major should have V (D major) in moderate progressions"
        );
    }
}
...
fn test_chord_progressions_from_with_extensions() {
    ...
    assert!(
        !options.is_empty(),
        "Should find progression options for Cmaj7"
    );
    ...
    assert!(
        !options.is_empty(),
        "Should find progression options for Dm7"
    );
}
