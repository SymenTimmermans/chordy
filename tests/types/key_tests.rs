...
fn test_key_degree_of_a_minor() {
    ...
    assert_eq!(
        a_minor.degree_of(note!("C")),
        ScaleDegree::new(3, Some(Accidental::Flat))
    );
    assert_eq!(a_minor.degree_of(note!("D")), ScaleDegree::new(4, None));
    assert_eq!(a_minor.degree_of(note!("E")), ScaleDegree::new(5, None));
    assert_eq!(
        a_minor.degree_of(note!("F")),
        ScaleDegree::new(6, Some(Accidental::Flat))
    );
    assert_eq!(
        a_minor.degree_of(note!("G")),
        ScaleDegree::new(7, Some(Accidental::Flat))
    );
}
