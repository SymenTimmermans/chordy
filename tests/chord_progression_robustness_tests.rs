...
fn test_simple_chords_match_simple_nodes() {
    ...
    for (expected_name, chord) in simple_chords {
        ...
        assert!(!options.is_empty(), 
               "Should have progression options for {} chord", expected_name);
        ...
        for chord in options.all().map(|(c, _)| c) {
            ...
            assert!(chord.bass.is_none() || chord.bass.is_some(), 
...
fn test_deterministic_progression_behavior() {
    ...
    for i in 0..10 {
        ...
        let strong_names: Vec<String> = options.strong.iter()
            .map(|c| c.to_html())
            .collect();
        let moderate_names: Vec<String> = options.moderate.iter()
            .map(|c| c.to_html())
            .collect();
        let weak_names: Vec<String> = options.weak.iter()
            .map(|c| c.to_html())
            .collect();
    ...
    for (i, result) in all_results.iter().enumerate() {
        assert_eq!(result.0, first_result.0, 
                  "Strong progressions should be identical on iteration {}", i);
        assert_eq!(result.1, first_result.1, 
                  "Moderate progressions should be identical on iteration {}", i);
        assert_eq!(result.2, first_result.2, 
                  "Weak progressions should be identical on iteration {}", i);
    }
...
fn test_slash_chord_vs_inversion_matching() {
    ...
    assert!(root_opts.as_ref().unwrap().len() > 0, "C root should have progression options");
    assert!(slash_f_opts.as_ref().unwrap().len() > 0, "C/F should have progression options");
    assert!(slash_e_opts.as_ref().unwrap().len() > 0, "C/E slash should have progression options");
    assert!(inv_opts.as_ref().unwrap().len() > 0, "C first inversion should have progression options");
}
...
fn test_progression_categories_populated() {
    ...
    for (name, chord) in test_chords {
        ...
        assert!(!options.moderate.is_empty(), 
               "{} should have moderate progression options", name);
        ...
        assert_eq!(iter_count, total, "Iterator count should match vector sum for {}", name);
    }
}
...
fn test_dominant_resolution() {
    ...
    let has_tonic_resolution = options.strong.iter().any(|chord| {
...
fn test_multiple_keys_consistency() {
    ...
    for key in keys {
        ...
        assert!(!options.moderate.is_empty(), 
               "Tonic should have moderate progressions in {}", key);
