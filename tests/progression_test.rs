...
fn test_chord_api() {
    ...
    assert!(
        ...
        "There should be strong progression options from G to C"
    );
    ...
    assert!(
        options.strong.contains(&c_major_chord),
...
fn test_major_key_progressions_from() {
    ...
    let has_f = options
        .strong
        .iter()
        .any(|chord| chord.root() == note!("F")); // IV
    let has_g = options
        .strong
        .iter()
        .any(|chord| chord.root() == note!("G")); // V

    assert!(
        has_f || has_g,
        "I should have strong progressions to IV or V"
    );

    // Should have moderate progressions too
    assert!(
        !options.moderate.is_empty(),
        "I should have moderate progression options"
    );
}
...
fn test_minor_key_progressions_from() {
    ...
    assert!(
        ...
        "i should have some progression options"
    );
...
fn test_progression_strength_categories() {
    ...
    assert!(
        has_c || has_e || has_a,
        "V should have strong progression to I, iii, or vi"
    );

    // Should also have moderate and weak options
    assert!(
        !options.moderate.is_empty(),
        "V should have moderate progression options"
    );
    assert!(
        !options.weak.is_empty(),
        "V should have weak progression options"
    );
}
