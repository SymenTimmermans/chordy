...
fn test_progressions_to_tonic() {
    ...
    let has_leading_tone = options.strong.iter().any(|chord| {
        chord.root == note!("B") && chord.intervals.contains(Interval::MINOR_THIRD)
    });
...
fn test_progressions_to_dominant() {
    ...
    let options = c_major.progressions_to(&dominant)
        .expect("Should find progressions leading to V");
    
    // ii should strongly lead to V (ii-V motion)
    let has_supertonic = options.strong.iter().any(|chord| {
        chord.root == note!("D") && chord.intervals.contains(Interval::MINOR_THIRD)
    });
    
    // IV might also lead to V
    let has_subdominant = options.strong.iter().any(|chord| {
        chord.root == note!("F") && chord.intervals.contains(Interval::MAJOR_THIRD)
    });
    
    assert!(has_supertonic || has_subdominant, 
...
fn test_progressions_to_extended_chords() {
    ...
    let options = c_major.progressions_to(&cmaj7)
...
fn test_progressions_to_minor_key() {
    ...
    let options = a_minor.progressions_to(&tonic)
        .expect("Should find progressions to i in minor");
    
    // V should lead to i (E major to A minor)
    let has_dominant = options.strong.iter().any(|chord| {
        chord.root == note!("E") && chord.intervals.contains(Interval::MAJOR_THIRD)
    });
    
    // Also check for v (natural minor)
    let has_minor_dominant = options.strong.iter().any(|chord| {
        chord.root == note!("E") && chord.intervals.contains(Interval::MINOR_THIRD)
    });
    
    assert!(has_dominant || has_minor_dominant, 
...
fn test_progressions_to_deterministic() {
    ...
    for _ in 0..10 {
        let options = key.progressions_to(&chord)
    ...
    for (i, result) in results.iter().enumerate() {
        assert_eq!(result, first, 
                  "progressions_to iteration {} differs from first", i);
    }
}
...
fn test_progressions_from_vs_to_comparison() {
    ...
    let from_iv = c_major.progressions_from(&f_chord)
        .expect("Should find progressions from IV");
    
    // What leads TO IV?
    let to_iv = c_major.progressions_to(&f_chord)
        .expect("Should find progressions to IV");
    
    // Both should have content
    assert!(!from_iv.is_empty(), "IV should have forward progressions");
    assert!(!to_iv.is_empty(), "IV should have progressions leading to it");
...
fn test_deprecated_progressions_from() {
    ...
    assert_eq!(old_options.is_some(), new_options.is_some(),
